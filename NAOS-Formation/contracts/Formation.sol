...
contract Formation is  ReentrancyGuard {
  ...
  /// @dev Resolution for all fixd point numeric parameters which represent percents. The resolution allows for a
  ...
  IMintableERC20 public token;//DAI

   /// @dev The token that this contract is using as the child asset.
  IMintableERC20 public xtoken;//nUSD
  ...
  /// @dev Sets the collateralization limit.//抵押限制
  ///
  ...
  //把生的利息(_vault.harvest)拿回來,一部分(_feeAmount)丟到reward addr,一部分(_distributeAmount)transmuter
  ...
  /// @dev Deposits collateral(抵押品) into a CDP.
  ///
  /// This function reverts if an emergency exit is active. This is in place to prevent the potential loss of
  /// additional funds.
  ///
  ...
  function deposit(uint256 _amount) external nonReentrant noContractAllowed expectInitialized {
    ...
    token.safeTransferFrom(msg.sender, address(this), _amount);//from msg.sender to this
    if(_amount >= flushActivator) {
      flushActiveVault();//把錢丟到vault
    }
